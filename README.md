...
- **帮助文案**用实际前缀渲染（如 `#搜本`、`#搜图 <关键词>`、`#搜图帮助`、`#搜本帮助`）；配置多个前缀时取**第一个**。
...
├── core/
│   ├── __init__.py
│   ├── image_source.py          # 图片获取与规范化（URL/本地/data URI → bytes；含 has_image 结构检测）
...
### soutubot.moe（以图搜图 / `搜本`）

- 仅由 `搜本` 触发（别名 `搜本子` / `soutu` / `找图`）；`搜图` 不再触发 soutubot
- 只接受图片（消息图片 / 引用图片 / `搜本 <图片链接>`）；纯关键词会回引导、不搜索
